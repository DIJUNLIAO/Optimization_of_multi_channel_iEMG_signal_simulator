...
while (((p_p(1)^2+p_p(2)^2 <= Rmuscle^2) && (p_p(3) > 0) && (p_p(3) < 30)))
% while (1)
    clearvars -except g g_prev p_prev p_current p_p Q alpha g_ratio loop_nm p_best p_p dt dz Lmuscle mf_center mnap_delays nmj_z Rmf Rmuscle sf sigma_i sigma_r sigma_z v Y object_value_all opt_mode muap_first alpha_value nmj_jitter MU_number
    ...
    % BFGS method to approach inverse Hessian
    gama=(g-g_prev)';
    delta=(p_p-p_prev)';
    Q=Q - ((delta*(gama')*Q+Q*gama*(delta'))/((delta')*gama))+(1+(((delta')*Q*delta)/((delta')*gama)))*((delta*(delta'))/(delta'*gama));
    ...
    if (loop_nm>=2) && (abs(objective_value-object_value_all(end-1)) <=10e-8)  %if the step sizes are too small
       p_best = p_p; 
       loop_nm=loop_nm+1;
       break
    end
